...
void showWorm(struct board* aboard, struct worm* aworm) {
    ...
    int tailindex = (aworm->headindex + 1) % (x);

    placeItem(aboard, aworm->wormpos[tailindex].y, aworm->wormpos[tailindex].x, BC_USED_BY_WORM, SYMBOL_WORM_TAIL_ELEMENT, aworm->wcolor);
}
...
void moveWorm(struct board* aboard, struct worm* aworm, enum GameStates* agame_state) {
    ...
    // the display if we move the worm’s head according to worm’s last direction.
